...
for i in str:
   dic[i]+= 1    # char is used as the key and the value is the count oft hat char in the string repeated chars hash to the same position
print(dic)

# Print characters with count > 1
for i, count in dic.items():
   if count > 1 and i != ' ':
      print ("%c is repeated %d times" % (i, count))
